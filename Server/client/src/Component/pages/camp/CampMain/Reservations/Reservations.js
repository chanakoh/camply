...
function Reservations() {
    ...
    const fetchCampingData = async () => {
      try {
        const seoulResponse = await axios.get(
          `http://mycamply.shop:8080/camp/board/location/서울`
        );
        const gyeonggiResponse = await axios.get(
          `http://mycamply.shop:8080/camp/board/location/경기`
        );
        const kangwonResponse = await axios.get(
          `http://mycamply.shop:8080/camp/board/location/강원`
        );
  ...
  useEffect(() => {
    const fetchRecentData = async () => {
      try {
        const response = await axios.get("http://mycamply.shop:8080/camp/board/all");
