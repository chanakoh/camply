...
const InquiryDetail1 = () => {
    ...
    const fetchData = async () => {
      try {
        const questionResponse = await axios.get(
          `http://mycamply.shop:8080/shop/question/${questionNo}`
        );
  ...
  const updateComments = async () => {
    try {
      const commentsResponse = await axios.get(
        `http://mycamply.shop:8080/shop/question/comment/list/${questionNo}`
      );
  ...
  const handleDeleteClick = async () => {
    if (window.confirm("정말 삭제하시겠습니까?")) {
      try {
        await axios.delete(
          `http://mycamply.shop:8080/shop/question/delete/${questionNo}`
        );
        alert("삭제 완료");
        navigate("/shop/main");
      } catch (error) {
          ...
          {question ? (
            <>
              ...
              <tbody>
                ...
                <tr>
                  <td>
                    <div className='data-content'>{question.questionText}</div>
                  </td>
                </tr>
              </tbody>
              <div>
          ...
          <div className='comment-list'>
            ...
            {comments.map((comment) => (
              <div
                ...
                className='inquiry-comment-list'
              >
                <div
                  style={{
