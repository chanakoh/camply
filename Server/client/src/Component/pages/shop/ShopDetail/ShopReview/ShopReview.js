...
const ShopReview = () => {
    ...
    const fetchReviews = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `http://camply.store/shop/review/view/${productId}` // 경로 변경: question -> review
        );
  ...
  return (
    <>
      <h2
        style={{ textAlign: "center", marginTop: "50px", marginBlock: "50px" }}
      >
        상품리뷰
      </h2>
        ...
        <table summary='번호, 제목, 작성자, 작성일, 조회'>
          ...
          {reviews.map((review) => (
            <tbody key={review.reviewNo}>
              <tr className='nbg'>
                <td>
                  <div className='tb-center'>
                    <span className='reviewnum'>{review.reviewNo}</span>
                  </div>
                </td>
                <td>
                ...
                <td>
                  <div className='tb-center'>
                    <span className='review_board_showhits1'>
                      {review.reviewHit}
                    </span>
                  </div>
                </td>
              ...
              <tr
                className='ms_review_content_box cnt'
                style={{ display: "none" }}
              >
                <td colSpan={6}>
