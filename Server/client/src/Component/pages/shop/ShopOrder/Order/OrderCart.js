      ...
      try {
        ...
        if (!isNaN(numericProductId)) {
          const response = await axios.get(
            `http://mycamply.shop:8080/shop/detail/${numericProductId}`
          );
  ...
  const saveOrder = async () => {
    try {
      ...
      const response = await axios.post(
        "http://mycamply.shop:8080/shop/order/post",
        orderData
      );
      ...
      if (location.state?.product) {
        ...
        await axios.delete(`http://mycamply.shop:8080/shop/cart/delete/${cartId}`);
