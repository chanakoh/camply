...
const OrderList = () => {
    ...
    if (userId) {
      try {
        const response = await axios.get(
          `http://camply.store/shop/mypage/general/myorder/view/${userId}`
        );
  ...
  const handleDeleteMyOrder = async (orderNo) => {
    try {
      await axios.delete(
        `http://camply.store/shop/mypage/general/myorder/delete/${orderNo}`
      );
      fetchMyOrder();
                        ...
                        <tr key={index}>
                          <td>
                            <div className='tb-center'>
                              <div className='orderNum'>{order.orderNo}</div>
                            </div>
                          </td>
                          <td>
                            <div className='tb-center'>
            ...
            <div style={{ marginBottom: "50px" }} className='basket-totalprice'>
              ...
              <div className='totalprice-text'>
                <span style={{ color: "#FEA92A" }}>
                  {order.reduce(
                    (acc, order) =>
                      acc + order.totalPrice * order.orderProductAmount,
                    0
                  )}
                </span>
                원
