...
const SellerProduct = () => {
  ...
  useEffect(() => {
    if (token) {
      ...
      axios
        .get("http://mycamply.shop:8080/shop/mypage/productList", {
          headers: {
  ...
  const handleDeleteClick = async (productId) => {
    if (window.confirm("정말 삭제하시겠습니까?")) {
      try {
        if (token) {
          await axios.delete(
            `http://mycamply.shop:8080/shop/mypage/productDelete/${productId}`,
            {
              headers: {
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      const updatePromises = changedProducts.map((product) =>
        axios.put(
          `http://mycamply.shop:8080/shop/mypage/product/statusEdit/${product.productId}`,
          {
